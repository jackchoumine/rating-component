...
export class LiRating {
  ...
  watchHandlerForColor(newValue: string) {
    this.colorInside = newValue.includes("#")
      ? this.convertHexToRGB(newValue)
      : newValue;
  ...
  watchHandlerForStrokeColor(newValue: string) {
    this.strokeColorInside = newValue.includes("#")
      ? this.convertHexToRGB(newValue)
      : newValue;
  ...
  componentWillLoad() {
    ...
    this.colorInside = this.color.includes("#")
      ? this.convertHexToRGB(this.color)
      : this.color;
    // this.fillColorInside = this.fillColor.includes("#") ? this.convertHexToRGB(this.fillColor) : this.fillColor;
    this.strokeColorInside = this.strokeColor.includes("#")
      ? this.convertHexToRGB(this.strokeColor)
      : this.strokeColor;
  }
  ...
  setElementAsSelectedUnselected(svgElmtMain) {
    ...
    this.meterTag.style.setProperty(
      "--unselected-icon-bg-url",
      `url('data:image/svg+xml,` +
        unSelectedSvg["outerHTML"] +
        `') 0 / auto 100%`
    );
    this.meterTag.style.setProperty(
      "--selected-icon-bg-url",
      `url('data:image/svg+xml,` + selectedSvg["outerHTML"] + `') 0 / auto 100%`
    );
  }
  ...
  calculateCurrentRating(event) {
    ...
    if (this.fillMode.toLowerCase() === "full") {
      ...
      newRatingValue =
        newRate % 1 > 0.5 ? Math.ceil(newRate) : Math.floor(newRate) + 0.5;
    } else {
